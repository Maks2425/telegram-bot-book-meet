...
async def main() -> None:
    ...
    try:
        ...
        logger.error(f"❌ {e}")
        logger.error(
            "\n💡 Підказка для локальної розробки:\n"
            "1. Створіть файл .env в корені проекту\n"
            "2. Додайте рядок: BOT_TOKEN=ваш_токен_від_BotFather\n"
        ...
        logger.error(
            ...
            "Для деплою (Railway, Render, тощо):\n"
            "1. Перевірте, чи встановлена змінна оточення BOT_TOKEN в панелі управління\n"
            "2. Переконайтеся, що токен скопійовано повністю без пробілів\n"
            "3. Перезапустіть сервіс після додавання змінної\n\n"
            "Загальне:\n"
            "4. Отримайте токен від @BotFather в Telegram"
        )
        sys.exit(1)
